...
if uploaded_file is not None:
    ...
    if file_contents is not None:
        st.write("File contents (as DataFrame):")
        st.dataframe(file_contents)
        ...
        st.write("Dataframe after renaming same courses:")
        st.dataframe(dataframe)

        scheduled_timetable = schedule(dataframe)

        # # Create a text area
        # output_text = st.text_area("Output", value="Your output will appear here", height=200)

        # # You can update the text area with new content like this:
        # st.text_area("Output", value="New output content", height=200, key="unique_key")
